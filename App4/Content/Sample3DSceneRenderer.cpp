...
{
    m_degreesPerSecond += (scrollChange * 10.0f);
    m_totalTime += timer.GetElapsedSeconds() + static_cast<double>(mouseChange);

    // Rotate the cube.
    // Convert degrees to radians, then convert seconds to rotation angle.
    const float    radiansPerSecond = XMConvertToRadians(m_degreesPerSecond);
    const double   totalRotation    = m_totalTime * radiansPerSecond;
    const float    radians          = static_cast<float>(fmod(totalRotation, XM_2PI));
    const XMMATRIX modelRotation    = XMMatrixRotationY(-radians);
    float   size = mouseDown ? 2.0f : 1.0f;
    ...
    auto modelSize = XMMatrixScaling(size, size, size);
    modelTransform = XMMatrixMultiply(modelSize, modelTransform);
